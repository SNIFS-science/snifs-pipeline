...
fused_profile_kernel = cp.ElementwiseKernel(
    ...
    float val_spec = pv_core(local_xiii, 0.0f, 0.6f * w_val, 1.1f * w_val, 5.4f, 0.6f, 0.4f, 1.0f, 0.8f)
...
def build_neighbor_matrix(
    target_spaxel: int, offsets: np.ndarray, widths: np.ndarray, oversample_factor: int = 1, group_size: int = 15
) -> tuple:
    ...
    try:
        ...
        local_target = target_spaxel % group_size


        # Identify the 14 neighbor indices (both local 0-14, and global)
        local_indices = np.array([i for i in range(group_size) if i != local_target])
        ...
        O_arr = cp.array(offsets[local_indices].T, dtype=cp.float32)
        O1, O2, O3, O4, O5 = [O_arr[i].T[:, None] for i in range(5)]

        W_arr = cp.array(widths[local_indices].T, dtype=cp.float32)
        ...
        normalize_and_mask_kernel(
            spectral, maxes, curve, ELL_C0_1D, ELL_C1_1D, ELL_A_1D, ELL_B_1D, dim, step_size, spectral
        )
        ...
        testModel_4d = spectral.reshape(n_active, 1400, W_real, oversample_factor, H_real, oversample_factor).sum(
            axis=(3, 5)
        )
        del spectral
        ...
        cp_global_indices = cp.array(global_indices)
        actual_global_spaxel = cp_global_indices[spaxel_idx]

        sparse_design_rows = (actual_global_spaxel * 1400) + spec_idx
        ...
        return sparse_data.get(), sparse_design_rows.get(), flat_ccd_indices.get()
    finally:
...
def build_target_matrix(
    ...
    try:
        ...
        O_arr = cp.broadcast_to(cp.array(offsets[target_spaxel % 15][:, None], dtype=cp.float32), (5, n_active))
        O1, O2, O3, O4, O5 = [O_arr[i].T[:, None] for i in range(5)]

        W_arr = cp.broadcast_to(cp.array(widths[target_spaxel % 15][:, None], dtype=cp.float32), (5, n_active))
        W1, W2, W3, W4, W5 = [W_arr[i].T[:, None] for i in range(5)]
        ...
        for i in range(n_active):
            ...
            pert_out.append((sparse_data.get(), sparse_design_rows.get(), flat_ccd_indices.get()))
        return pert_out
    finally:
        cp.get_default_memory_pool().free_all_blocks()
        cp.get_default_pinned_memory_pool().free_all_blocks()
