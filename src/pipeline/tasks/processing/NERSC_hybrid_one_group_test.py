...
from pipeline.tasks.processing.CG_GPU_solver import cg_solve_chunked
try:
    import cupy as cp
...
def least_squares_diagnostics(A, b, x):
    residual_vector = A @ x - b
    
    res_norm = cp.linalg.norm(residual_vector)
    
    gradient = A.T @ residual_vector
    
    return res_norm, gradient


@task(name="Solve_Perturbation_GPU", **_TASK_KW)
def solve_perturbation_gpu_task(payload, p_idx):
    import gc
    import cupy as cp
    ...
    # Need to form AtA before moving to GPU
    data, cols, rows = assemble_triplets(payload["neighbor"], payload["perts"][p_idx])
    group_id = spax_id // SPAXELS_PER_GROUP
    base_col = group_id * SPAXELS_PER_GROUP * SPEC_LEN
    cols -= base_col
    ...
    A = sparse.csr_matrix(
        (data, (rows.astype(np.int32, copy=False), cols.astype(np.int32, copy=False))),
        shape=MATRIX_SHAPE,
    )
    del data, rows, cols
    gc.collect()
    ...
    try:
        # Form AtA and Atb on CPU
        At = A.T.tocsr()
        AtA = (At.dot(A)).tocsr() + 1e-10 * sparse.eye(MATRIX_SHAPE[1], format="csr")
        Atb = At.dot(b)
        del At
        gc.collect()

        # Sort indices while it is still a native CPU SciPy matrix
        AtA.sort_indices()

        # Transfer to GPU and solve (make sure to cast to 32 bit first)
        M_cp = cpsp.csr_matrix(
            (
                cp.array(AtA.data, dtype=cp.float64),
                cp.array(AtA.indices, dtype=cp.int32),
                cp.array(AtA.indptr, dtype=cp.int32),
            ),
            shape=AtA.shape,
        )
        c_cp = cp.array(Atb, dtype=cp.float64)
        
        # Try cholesky in place of cg_solve chunked and different max iters

        x_cp = cg_solve_chunked(M_cp, c_cp,max_iter=2000,check_interval=500)
        
        # back to CPU
        x = cp.asnumpy(x_cp)
        
        del M_cp, c_cp, x_cp, AtA, Atb
        if cp is not None:
            cp.get_default_memory_pool().free_all_blocks()

    except Exception as e:
        get_run_logger().error(f"GPU OOM/Error caught on Spaxel {spax_id}: {str(e)}")
        if cp is not None:
            cp.get_default_memory_pool().free_all_blocks()
        raise RuntimeError("Task failed due to GPU runtime error.") from e
    ...
    fit_model = A.dot(x)
    loss = float(np.sum(np.abs(science - fit_model)))
    bin_stats = compute_bin_stats(fit_model, science)
    del A, x, fit_model
        ...
        for group_id in range(NUM_GROUPS):
            ...
            for spax_id in range(group_id * SPAXELS_PER_GROUP, (group_id + 1) * SPAXELS_PER_GROUP):
                ...
                for p in range(n_pert):
                    if p % 2 == 2:
                        # every other task (index 1, 3, 5) goes to the GPU since there is a 16 to 32 worker ratio and GPUs are ~twice as fast
                        fut = _submit(solve_perturbation_gpu_task, gpu_addrs, "GPU", payload_fut, p)
                    else:
                        fut = _submit(solve_perturbation_task, cpu_addrs, "CPU", payload_fut, p)
                    sol_futs.append(fut)
                agg = _submit(
